...
export default function PurchaseTransactions() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await hrService.listTransactions({ transactionType: tab, direction: 'purchase', search, status: statusFilter, page, limit: 35, ...dateParams });
      setData(res.data || []);
  ...
  const columns = [
    ...
    { key: 'party_name', label: 'Supplier', render: (v) => v || '—' },
        ...
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div>
            ...
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 min-w-[120px]">
              ...
              {['draft', 'sent', 'paid', 'partial', 'overdue', 'approved', 'received', 'completed', 'issued'].map(s => (
