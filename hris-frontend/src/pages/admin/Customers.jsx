...
const emptyForm = {
  name: '', email: '', phone: '',
  gstin: '', pan: '',
  opening_balance: '', opening_balance_type: 'receivable', credit_period: '', credit_limit: '',
...
const Customers = () => {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      const res = await hrService.getCustomers(params);
  ...
  return (
    <div className="space-y-4">
            ...
            {formTab === 1 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">You will receive = they owe, You will give = you owe</p>
                </div>
                <div>
      ...
      <Drawer
        ...
        {selectedRecord && (detailTab === 'details' ? <CustomerDetailContent customer={selectedRecord} /> : <TransactionsTab partyType="customer" partyId={selectedRecord.id} partyName={selectedRecord.name} />)}
      </Drawer>
    </div>
  );
};
...
function CustomerDetailContent({ customer }) {
  const balType = customer.opening_balance_type === 'receivable' ? 'You will receive' : 'You will give';
  return (
    <div className="space-y-3">
      ...
      <DetailRow label="Opening Balance" value={customer.opening_balance ? `${formatINR(customer.opening_balance)} (${balType})` : '—'} />
      <DetailRow label="Credit Period" value={customer.credit_period ? `${customer.credit_period} days` : '—'} />
      <DetailRow label="Credit Limit" value={customer.credit_limit ? formatINR(customer.credit_limit) : '—'} />
      <DetailRow label="Billing Address">
        <div>{customer.billing_address}{customer.billing_address_line2 ? `, ${customer.billing_address_line2}` : ''}{customer.billing_city || customer.billing_address || customer.billing_postal_code ? <><br/>{[customer.billing_city, customer.billing_state, customer.billing_postal_code].filter(Boolean).join(', ')}{customer.billing_country ? <><br/>{customer.billing_country}</> : ''}</> : '—'}</div>
      </DetailRow>
      <DetailRow label="Shipping Address">
        <div>{customer.shipping_address}{customer.shipping_address_line2 ? `, ${customer.shipping_address_line2}` : ''}{customer.shipping_city || customer.shipping_address || customer.shipping_postal_code ? <><br/>{[customer.shipping_city, customer.shipping_state, customer.shipping_postal_code].filter(Boolean).join(', ')}{customer.shipping_country ? <><br/>{customer.shipping_country}</> : ''}</> : '—'}</div>
      </DetailRow>
